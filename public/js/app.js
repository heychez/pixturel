...
$("#myaudio").ready(function () {
	var audioDuration = audio.duration;
	var barMaxWidth = $("#audio-progress").width();
	var track = Math.floor((Math.random() * 7) + 1);

	if (track == 1) {
		audio.src = 'public/tracks/1.mp3';
		audioDuration = 69.01551;
	} else if (track == 2) {
		audio.src = 'public/tracks/2.mp3';
		audioDuration = 70.791837;
	} else if (track == 3) {
		audio.src = 'public/tracks/3.mp3';
		audioDuration = 58.618776;
	} else if (track == 4) {
		audio.src = 'public/tracks/4.mp3';
		audioDuration = 74.501224;
	} else if (track == 5) {
		audio.src = 'public/tracks/5.mp3';
		audioDuration = 67.056327;
	} else if (track == 6) {
		audio.src = 'public/tracks/6.mp3';
		audioDuration = 72.620408;
	} else {
		audio.src = 'public/tracks/7.mp3';
		audioDuration = 71.235918;
	}
	...
	function _animarPixel(pixel, l, t, i, b, v, s) {
		setTimeout(function () {
			pixel.set({
				left: l - i,
				top: t - i,
				width: PIXEL_WIDTH + i * 2,
				height: PIXEL_HEIGHT + i * 2
			});
			canvas.renderAll();
